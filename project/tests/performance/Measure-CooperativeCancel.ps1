...
if ([string]::IsNullOrWhiteSpace($ReceiptPath)) {
    ...
    $ReceiptPath = Join-Path $projectRoot "_work\performance-cooperative-cancel-$stamp.json"
}
$ReceiptPath = [IO.Path]::GetFullPath($ReceiptPath)
if (-not $ReceiptPath.StartsWith($repositoryRoot + '\', [StringComparison]::OrdinalIgnoreCase)) { throw 'ReceiptPath must be inside the repository root.' }
