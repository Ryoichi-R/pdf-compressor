...
if /i "%NATIVE_ARCH%"=="ARM64" (
    set "PDF_COMPRESSOR_RUNTIME=win-arm64"
    echo [INFO] ARM64 host detected. Selecting the native win-arm64 payload.
) else if /i "%NATIVE_ARCH%"=="AMD64" (
    echo [ERROR] This package contains only the native ARM64 payload. Use the x64 package on AMD64 Windows.
    pause
    exit /b 1
) else (
